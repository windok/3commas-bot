...
// strategy.risk.max_intraday_loss(maxTotalVolume * strategy.position_avg_price[0] * 0.98, strategy.cash)
...
if (isLongStrategy)
    ...
    // strategy.order("Long" + tostring(filledOrders + 1), strategy.long, qty=buyMoreQty, when=(absolutePosition > 0 and filledOrders < maxOrders and close <= buyMoreLongPrice), limit=buyMoreLongPrice)
